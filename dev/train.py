...
# get data
train_data = np.load('/home/shaowu/Desktop/Postdoc/PROJECTS/2022-oliver-pruning/3d-wing-flow/3d-wing-flow.npz')['data']
np.random.shuffle(train_data)

num_total_data = train_data.shape[0]

if enable_sobolov:
    train_dataset = tf.data.Dataset.from_tensor_slices((train_data[:, :5], train_data[:, 5:]))
else:
    train_dataset = tf.data.Dataset.from_tensor_slices((train_data[:, :5], train_data[:, 5:5 + n_output]))

...
# Time,    "Points:0","Points:1",     "x_velocity","y_velocity",     "Area"
train_data = np.load('../data/cylinder_uv_area_siren_n_eco.npz')['demo']
num_total_data = train_data.shape[0]
train_dataset = tf.data.Dataset.from_tensor_slices((train_data[:, :3], train_data[:, 3:5], train_data[:,-1])) # last
# one is sample weight
...
with cm:
    ...
    model_ori = nif.NIFMultiScaleLastLayerParameterized(cfg_shape_net, cfg_parameter_net, mixed_policy)
